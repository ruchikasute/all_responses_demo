...
def get_scope_prereq_assumptions_prompt(reference_text, condensed_rfp, num_interfaces=None):
    """Ultra-light version — same Crave structure, no hardcoded bullets, faster and cleaner."""
    interface_info = (
        f"Total ICOs – {num_interfaces}"
        if num_interfaces
        else "Migration of interfaces from SAP PI/PO to SAP Integration Suite cloud integration"
    )

    return f"""
You are an SAP proposal writer at Crave InfoTech.

Write the following four sections exactly in this format (bullets only, no paragraphs):

In Scope:
• Mention migration of interfaces from SAP PI/PO to SAP Integration Suite cloud integration
• {interface_info}
• 4–6 relevant points from the RFP

Migration Project Prerequisites:
• 4–6 concise bullet points describing technical and client-side prerequisites

Assumptions:
• 5–8 assumption bullets derived from the RFP and reference text

Out of Scope:
• 4–6 clear bullet points describing exclusions from the migration project

Keep the tone formal, client-facing, and consistent with Crave InfoTech proposals.
Use “the client” or client name (if available in context).
